...
# ╔═╡ 269c5e6a-4611-4b32-985b-c4d6b5b5dd30
begin
	using Reel
	function create_exp_gif(mxs; name="day9_exploration")
		sim_frames = Frames(MIME("image/png"), fps=100000000)
		for i in 1:length(mxs)
			frame_i = heatmap(.!mxs[i], colors=1, legend=:none)
			push!(sim_frames, frame_i)
		end
		!isdir("gifs") && mkdir("gifs") # create "gifs" directory
		write("gifs/$(name).gif", sim_frames)
	end
end
...
# This is DFS
function find_basin_size((row, col), data_size, floor_heights; explored=zeros(Bool, data_size...), explored_history = nothing)
...
function find_basin_sizes_simple(floor_heights; explored_history = nothing)
	data_size = size(floor_heights)
	(total_rows, total_cols) = data_size
	return [find_basin_size((row, col), data_size, floor_heights; explored_history = explored_history) for col in 1:total_cols for row in 1:total_rows]
end

# ╔═╡ ecf9b106-c7a9-4781-90cc-56ab4b79c4e3
md"""
...
# ╔═╡ e88b2d90-9106-45b3-9ec1-4d189c3a0a19
md"""
...
function find_basin_sizes_optim(floor_heights; explored_history=nothing)
	...
	for lp in lowest_points
		...
		push!(basin_sizes, find_basin_size(lp, data_size, floor_heights; explored_history=lp_explored))
...
# ╔═╡ 978fdfd6-a481-46d5-88f4-5ae03c27ebf8
...
with_terminal() do
	open("./Day9/prob_input.txt") do io
		floor_heights = parse_file(io)
		explored_history = []
		@time basin_sizes = find_basin_sizes_optim(floor_heights; explored_history = explored_history) |> sort
		if create_gif
